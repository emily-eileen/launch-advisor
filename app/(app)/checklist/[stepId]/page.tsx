...
import {
  ...
  BookOpen, Loader2, Zap, FileText, Sparkles, Diamond,
...
const PREMIUM_FEATURES: {
...
}[] = [
  {
    ...
    tagline: "Get a step-by-step plan tailored to your business type and where you are right now.",
  ...
  },
  {
    ...
    tagline: "AI pulls competitor data, market size estimates, and demand signals for your niche.",
    cta: "Run research",
  },
  {
    id: "logo-brand-kit",
    ...
    tagline: "Generate logo concepts, a color palette, and font pairings — ready to use in minutes.",
    cta: "Build my brand",
  },
  {
    id: "customer-survey",
    icon: ClipboardList,
    label: "Customer Survey Builder",
    tagline: "AI writes discovery questions specific to your industry, then analyzes the responses.",
    cta: "Build my survey",
  },
  {
    ...
    tagline: "See who you're up against, what they charge, and where the gaps are in your market.",
  ...
  },
  {
    ...
    tagline: "Full AI-drafted business plan with executive summary, financials, and market positioning.",
    cta: "Draft my plan",
  },
];
...
export default function StepDetailPage() {
  ...
  useEffect(() => {
    ...
    if (savedProgress && step) {
      ...
      setIsDone(set.has(step.id));
    }

    // Load quiz business type for guide filtering
    const quiz = localStorage.getItem("launchadvisor_quiz");
    if (quiz) {
      try {
        const parsed = JSON.parse(quiz);
        setQuizBusinessType(parsed.businessType ?? null);
      } catch { /* ignore */ }
    }

    // Load saved journal entry for this step
    if (step) {
  ...
  // Filter guides by business type from quiz (guides without businessTypes show for everyone)
  ...
  async function handleAiJournal() {
    // Premium AI feature — generates a journal entry draft based on the step
    setAiLoading(true);
    await new Promise((r) => setTimeout(r, 1800));
    // Placeholder: in production this calls /api/ai/journal with step context
    const draft = `Based on the goal of "${step!.title.toLowerCase()}", here's a starting point: I'm thinking through who exactly my ideal customer is and what problem they urgently want solved. The key question I need to answer is whether this problem is painful enough that someone would actually pay to fix it today, not just say "that'd be nice."`;
  ...
  return (
    <div style={{ maxWidth: 680, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div className="panel" style={{ borderLeft: `4px solid ${color}`, padding: 28, boxShadow: `4px 4px 0 ${color}` }}>
        ...
        {/* Description */}
        <p style={{ fontFamily: "var(--font-body)", color: "var(--ink-mid)", lineHeight: 1.7, marginBottom: 20, fontSize: "0.97rem" }}>
          {step.description}
        </p>

        {/* Do this now */}
        <div style={{
          background: `${color}0E`, border: `1.5px solid ${color}33`,
          padding: "14px 16px",
        }}>
      ...
      {/* ── Journal this step ─────────────────────────────────── */}
      <div className="panel" style={{ padding: 24, borderLeft: `3px solid ${color}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <BookOpen style={{ width: 14, height: 14, color }} />
            <span style={{
          ...
          {/* AI "Do it for me" — premium feature */}
          <button
            ...
            style={{
              ...
              background: aiLoading ? "#7C3AED18" : "transparent",
              color: "#7C3AED",
            ...
            title="AI generates a journal draft for you — premium feature"
          >
            {aiLoading
        ...
        <p style={{
          fontFamily: "var(--font-body)", fontSize: "0.84rem", color,
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
          ...
          <span style={{
            fontFamily: "var(--font-display)", fontSize: "0.58rem", letterSpacing: "0.18em",
          ...
          {PREMIUM_FEATURES.map((feat) => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => {
                  (e.currentTarget as HTMLElement).style.background = "#7C3AED12";
                  (e.currentTarget as HTMLElement).style.borderColor = "#7C3AED66";
                }}
                onMouseLeave={e => {
                  (e.currentTarget as HTMLElement).style.background = "#7C3AED06";
              ...
              >
                ...
                <p style={{
                  ...
                  color: "var(--navy)", marginBottom: 5, lineHeight: 1.2,
                }}>
                  {feat.label}
                </p>
                <p style={{
                  fontFamily: "var(--font-body)", fontSize: "0.73rem", color: "var(--ink-muted)",
                ...
                <span style={{
                  ...
                  letterSpacing: "0.06em", textTransform: "uppercase", color: "#7C3AED",
                  borderBottom: "1.5px solid #7C3AED44", paddingBottom: 1,
                }}>
                  {feat.cta} →
                </span>
              </div>
      ...
      {/* ── Go deeper: guide articles (filtered by quiz type) ─── */}
      {filteredGuides.length > 0 && (
        <div>
          ...
          {quizBusinessType && filteredGuides.length < relatedGuides.length && (
            <p style={{
              fontFamily: "var(--font-body)", fontSize: "0.72rem", color: "var(--ink-muted)",
              marginTop: 10, textAlign: "center",
            }}>
              Showing guides for {businessTypeLabel}.{" "}
              <button
                onClick={() => setRelatedGuides(relatedGuides)}
                style={{ background: "none", border: "none", cursor: "pointer", color, fontFamily: "var(--font-heading)", fontSize: "0.72rem", fontWeight: 600, textDecoration: "underline" }}
              >
            ...
            {step.resources.map((resource, idx) => (
              <div key={idx} style={{
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontFamily: "var(--font-heading)", fontSize: "0.95rem", fontWeight: 700, color: "var(--navy)" }}>
                      {resource.name}
                    </span>
                    {resource.badge && (
                      <span style={{
                  ...
                  {resource.affiliate && (
                    <span style={{
                      ...
                      textTransform: "uppercase", color: "var(--ink-muted)", flexShrink: 0,
                    }}>
                      affiliate
                    </span>
                  )}
                ...
                {/* Description — fuller context, natural language */}
                {resource.description && (
                  <p style={{
                    fontFamily: "var(--font-body)", fontSize: "0.84rem", color: "var(--ink)",
      ...
      <div style={{
        ...
        <div style={{
          width: 38, height: 38, background: `${color}14`, border: `1.5px solid ${color}44`,
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{
            fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--ink-muted)",
